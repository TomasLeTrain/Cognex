...
void odom_tuning() {
  ...
  while (true) {
    ...
    auto odom_to_diameter = [target_distance_in](double pos) -> Length {
      ...
      const Length expected_diameter = target_distance_in / (M_PI * rotations);
...
void drive_pid_tuning() {
  ...
  while (true) {
    ...
    mb.moveTo(target_distance, 0_in).drive_kp(curr_kp).drive_kd(curr_kd) | run;
    ...
    std::cout << std::format(
                     ...
                     total_error, forwards_error, sideways_error)
              << std::endl;
    ...
    while (!controller.get_digital_new_release(pros::E_CONTROLLER_DIGITAL_A)) {
      if (controller.get_digital_new_release(pros::E_CONTROLLER_DIGITAL_LEFT)) {
        ...
        std::cout << std::format("decreased target to {}", target_distance)
                  << std::endl;
      }
      if (controller.get_digital_new_release(
              pros::E_CONTROLLER_DIGITAL_RIGHT)) {
        target_distance += target_distance_delta;
        std::cout << std::format("increased target to {}", target_distance)
                  << std::endl;
      }

    ...
    pros::Task lemlib_pose_task { [&] {
        while (true) {
            ...
            if (orientation_getter != nullptr) {
                Angle other_orientation =
                  orientation_getter->getPose().orientation;
            ...
            // its likely fine to set infinity values since that likely make the
            // program stop its movement as compared to not updating the pose
            chassis.setPose(to_in(curr_pose.x),
                            to_in(curr_pose.y),
                            to_cDeg(curr_orientation),
                            false);

            // performed quickly to get the latest information as soon as
            // possible

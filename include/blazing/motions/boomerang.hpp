...
template <typename ControllersType, typename DrivetrainType,
          typename TrackerType, typename TolerancesType>
...
class boomerang : public Motion<ControllersType, DrivetrainType, TrackerType,
                                TolerancesType>,
                  public LinearMotion,
...
private:
  ...
  std::function<double(Angle)> angular_linear_func = [](Angle angle) -> double {
...
public:
  int getLoopDelayTime() override { return 10; }
  ...
  std::optional<motionExecutionResult> execute() override {
    ...
    // heading already reversed, shouldn't need to reverse here?
    ...
    if (units::abs(pose_target_distance) < close_threshold && !state.close) {
      state.close = true;
    }
    ...
    const units::V2Position carrot = [&] -> units::V2Position {
      if (state.close)
        return target;
      auto carrot =
          target - units::V2Position::fromPolar(target_orientation,
      ...
      Length sideways_error =
          (target - position) * units::Vector2D{-units::sin(target_orientation),
                                                units::cos(target_orientation)};

      if (!state.initial_side)
        state.initial_side = sideways_error;

      Length abs_sideways_error = units::abs(sideways_error);

      if (state.crossed_sideways || abs_sideways_error < lead2_dist_threshold) {
      ...
      auto lead2_vector = units::V2Position::fromPolar(
          target_orientation + 90_stDeg, pose_target_distance * m_lead2);
    ...
    Length linear_error = position.distanceTo(carrot) * (reversed ? -1.0 : 1.0);

    Angle angular_error = angleError(target_heading, heading);

    // why is this different???
    Angle position_carrot_error = angleError(position_carrot_heading, heading);
    ...
    this->tolerances.linearHalfcircleToleranceUpdate(position, target, heading);
    ...
    if constexpr (hasLargeLinearTolerance<TolerancesType>) {
      result.inLargeTolerance = this->tolerances.large_linear.withinTolerance();
    ...
    if constexpr (hasChainLinearTolerance<TolerancesType>) {
      result.inChainTolerance = this->tolerances.chain_linear.withinTolerance();
    }
    ...
    Voltage angular_output = this->controllers.angular_feedback.update(
        -angular_error, 0_stRad, delta_time);

    Voltage linear_output = this->controllers.linear_feedback.update(
        -linear_error, 0.0_in, delta_time);
  ...
  boomerang(ControllersType controllers,
      ...
      : boomerang(controllers, chassis, from_in(x), from_in(y),
                  from_stDeg(heading)) {}

  boomerang &getReference() { return *this; }
  ...
  [[nodiscard("motion won't be executed unless an executor is used!")]]
  auto
  k_lat(std::optional<std::variant<Divided<Angle, Length>, double, int>> k_lat =
            std::nullopt,
